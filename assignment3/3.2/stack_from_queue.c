...
TYPE listQueueFront(struct Queue* queue) {
	...
	if (listQueueIsEmpty(queue) == 0) {
		return queue->head->next->value;
	}
	else {
		printf("Cannot return front value. Size is 0.\n\n");
...
TYPE listQueueRemoveFront(struct Queue* queue) {
	...
	if (listQueueIsEmpty(queue) == 0) {
		struct Link* holder = queue->head->next;
		queue->head->next = queue->head->next->next;
		retValue = holder->value;

		if (queue->tail == holder) {
		...
		printf("Cannot remove front. Size is 0.\n\n");
		retValue = queue->head->value;
		return retValue;
	}
